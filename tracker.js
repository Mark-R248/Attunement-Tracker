...
.charTab:hover { background: #d0d0d0; }
...
.charTab .tabClose:hover { opacity: 1; color: #c00; }
...
#addCharBtn:hover { background: #e8e8e8; }

#charPanel { padding: 10px; }
...
.charNameInput:focus { border-bottom-color: #888; }
...
.attAddRow select, .attAddRow input, .attAddRow button {
  padding: 6px 8px;
  font-size: 13px;
  font-weight: bold;
  border: 1px solid #aaa;
...
.compoundCard.active { background: #6b6b6b; color: white; }
.compoundCard.glow   { box-shadow: 0 0 6px #aaa; }
...
.compoundCard.active .compoundSubTypes { opacity: 0.75; }
.compoundBtn .compoundSubTypes { font-size: 9px; opacity: 0.55; margin-top: 1px; }
...
<div id="topBar">
  <strong style="font-size:14px">Characters:</strong>
  <button onclick="addCharacter()">New Character</button>
  <div class="sep"></div>
  <button onclick="exportSave()">Export Save</button>
  <button onclick="document.getElementById('importFileInput').click()">Import Save</button>
  <input type="file" id="importFileInput" accept=".json" onchange="importSave(event)" />
  <div class="sep"></div>
  <button id="undoBtn" onclick="undo()" style="opacity:0.5" title="Undo last cast or emulation">↩ Undo</button>
</div>

<div id="charTabBar">
  <button id="addCharBtn" onclick="addCharacter()">+ Add Character</button>
</div>

<div id="charPanel">
  <div style="text-align:center;padding:40px;color:#888;font-size:15px;">
    No characters yet. Click <strong>+ New Character</strong> to get started.
  </div>
</div>
...
const RANK_THRESHOLDS = [
  ...
  [2160,   "Topaz",    480],
  [12960,  "Emerald",  1920],
  [77760,  "Sapphire", 11520],
  [466560, "Amethyst", 138240],
];
...
function renderWhisperEmulateRow(charId, att) {
  ...
  costNote.textContent = `Cost: ${tier.cost}`;
